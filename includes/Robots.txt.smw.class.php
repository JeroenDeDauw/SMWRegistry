<?php
  ...
  /* Edits by Tobias Matzner : marked with comments like this //tm: ...
   * Added a function to check wether a crawl-delay is given
   */
...
class Robots_txt {
	...
	protected function init($strScheme,$strHost,$strUserAgent) {
		if( (strlen(($this->strUserAgent=$strUserAgent))) && (strlen(($this->strScheme=$strScheme))) && (strlen(($this->strHost=$strHost)))) {
			ini_set('error_reporting', E_ALL & ~E_NOTICE & ~E_WARNING); // hide "no robots.txt" warnings!
			...
			if((($intCount=count((($arrRules=explode("\n",$strRobotsFile))))))) {
				...
				for($intCounter=0; $intCounter<$intCount; $intCounter++) {
					...
					if( (strlen(($strLine=trim($arrRules[$intCounter])))) && (!(preg_match('/^\s*#.*$/',$strLine)))) {
						...
						#I know, the strpos function may return 0, but if the : is the first character, I cant use the input anyway

						if(strpos($strLine,':')) {

							$arrNameValuePair=explode(':',$strLine,2); //tm: added limit, otherwise URL including ":" like MediaWiki Special Pages will not be treated right.
	...
	public static function isSupportedScheme($strUrl,array &$arrResult=null) {
		...
		if(strlen($strUrl=trim($strUrl))) {//tm:removed downcasing
	...
	public static final function urlAllowed($strUrl,$strUserAgent=null) {
		...
		return $blOut;
			...
			/*check whethet a crawl delay is given for user agent
			 *params: $strUrl: url to check, $strUserAgent: name of user agent
			 */

	public static final function crawlDelay($strUrl,$strUserAgent) {
